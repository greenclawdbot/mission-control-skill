...
async function main() {
  ...
  if (task) {
    ...
    console.log(`1. Do the work`);
    console.log(`2. Record results to task:`);
    console.log(`   curl -X PATCH "${API_URL}/api/v1/tasks/${task.id}" \\`);
    console.log(`     -H "Content-Type: application/json" \\`);
    console.log(`     -d '{"results": "## Summary\\n- Did X\\n- Did Y"}'`);
    console.log(`3. Move to Review:`);
    console.log(`   curl -X PATCH "${API_URL}/api/v1/tasks/${task.id}" \\`);
    console.log(`     -H "Content-Type: application/json" \\`);
    console.log(`     -d '{"status": "Review"}'`);
    console.log(`",`);
